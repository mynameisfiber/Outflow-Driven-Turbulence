...
call analysis_init(30,20,oImp**(4.0/7.0)*(oSnorm0)**(3.0/7.0),node,odir,analysisTodo)
...
CONTAINS
  ...
  SUBROUTINE gridanalysis(u,n,dt,t,numinject,maxv)
    ...
    INTEGER :: n, newinject, numinject, i,j,k, idx, d, chk, tmpidx
    LOGICAL :: isinject 
    ...
    if (doanalysis) call analysis_calc_init()
    ...
    !$OMP PRIVATE(i,j,k,oi,oj,ok,ci,cj,ck,tmpidx) DEFAULT(SHARED) &
...
!          END IF
          ...
          rand() .LE. 2*dt*oSnorm0*(u(i,j,k,1)**on) ) THEN
            ...
            tmpidx = idx
            idx = idx + 1
            lflows(:,tmpidx) = (/ oi, oj, ok, ci, cj, ck, REAL(node) /)
    ...
    END DO
    totalrho = ltotalmass
    meanrho = ltotalmass / (n-2*ghost)**3
